...
const POSITION_CARDS = [
  ...DEPTH_POSITIONS_ORDER.offense.map((position) => ({
    position,
    group: "Offense",
  })),
  ...DEPTH_POSITIONS_ORDER.defense.map((position) => ({
    position,
    group: "Defense",
  })),
  ...DEPTH_POSITIONS_ORDER.special.map((position) => ({
    position,
    group: "Special Teams",
  })),
];
...
function groupPlayers(players) {
  ...
  players.forEach((player) => {
    const key = KNOWN_POSITIONS.has(player.depth_position)
  ...
  Object.values(grouped).forEach((list) =>
    list.sort(
      (a, b) => (a.depth_order ?? Infinity) - (b.depth_order ?? Infinity),
    ),
  );
...
function findContainerOf(playerId, grouped) {
  return Object.keys(grouped).find((key) =>
    grouped[key].some((p) => p.id === playerId),
  );
}
    ...
    <div ref={setNodeRef} className={styles.card}>
      ...
      >
        <div className={styles.cardBody}>
          {players.length === 0 && (
            <p className={styles.cardEmpty}>No players</p>
          )}
          {players.map((player) => (
...
export default function DepthChartBoard({ players, onSaved }) {
  ...
  const handleDragOver = (event) => {
    ...
    const overContainer = grouped[over.id]
      ? over.id
      : findContainerOf(over.id, grouped);
  ...
  const handleDragEnd = (event) => {
    ...
    const overIndex = grouped[over.id]
      ? items.length - 1
    ...
    reordered.forEach((player, index) => {
      ...
      if (
        player.depth_position !== newDepthPosition ||
        player.depth_order !== nextOrder
      ) {
        saveRecord("players", {
          id: player.id,
        ...
        <span className={styles.toolbarLeft}>
          {hiddenIds.size > 0 && (
            <span className={styles.hiddenNote}>
              ...
              <button
                type="button"
                className={styles.hiddenShowBtn}
                onClick={unhideAll}
              >
                show all
              </button>
