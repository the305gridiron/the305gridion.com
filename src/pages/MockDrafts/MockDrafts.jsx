...
export default function MockDrafts() {
    ...
    const draftVersions = useMemo(() => {
        if (!data) return [];
        return Array.from(new Map(data.map(d => [
            `${d.version}-${d.date}`,
            { id: d.version, year: new Date(d.date).getFullYear(), date: d.date }
        ])).values());
    }, [data]);

    const requestedYear = Number(searchParams.get("year"));
    const requestedVersion = Number(searchParams.get("version"));
    const validVersion = draftVersions.find((draft) => draft.year === requestedYear && draft.id === requestedVersion);
        ...
        <>
            ...
            <div className={styles.mockDrafts}>
                <Hero>
                    ...
                    <Hero.Promo>
                        Who doesn't love the chaos of draft season? We're breaking down
                        the best fits, the biggest surprises, and the names you need to
                        know for the 2026 NFL Draft.
                    </Hero.Promo>
                ...
                <main className="container-fluid">
                    ...
                    <Sidebar id="mockDrafts">
                        <SidebarCards>
                            <SidebarCard>
                                <SidebarCard.Title>2026 Mock Drafts</SidebarCard.Title>
                                <SidebarCard.List>
                                    {draftVersions.map((v) => (
                                        <SidebarCard.ListItem key={`${v.year}-${v.id}`}>
                                            {v.id === currentDraft?.id && v.year === currentDraft?.year ? (
                                                <strong>
                                                    Version {v.id} • {formatDate(v.date)}
                                                </strong>
                                            ) : (
                                                <Link to={`/mocks?year=${v.year}&version=${v.id}`}>
                                                    Version {v.id} • {formatDate(v.date)}
                                                </Link>
                                            )}
