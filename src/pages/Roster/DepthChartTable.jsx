              ...
              return (
                <React.Fragment key={category}>
                  <tr className={styles.categoryDividerRow}>
                    <td colSpan={7} className={styles.categoryDividerName}>
                      {category === "special"
                        ? "SPECIAL TEAMS"
                  ...
                  {positions.map((position) => {
                    ...
                    const playerRows =
                      activeTab === "inactive"
                        ? chunkPlayers(players, DEPTH_COLUMNS.length)
                        : [players];
                    ...
                    return playerRows.map((rowPlayers, rowIndex) => (
                      ...
                      >
                        <td className={styles.posLabelCell}>
                          <div className={styles.posLabelBadge}>
                            {rowIndex === 0 ? getPositionLabel(position) : ""}
                          </div>
                        </td>

                        {DEPTH_COLUMNS.map((depthIndex) => {
                          const player = rowPlayers[depthIndex];

                          if (!player) {
                            return (
                              <td
                                key={depthIndex}
                                className={styles.emptyDepthCell}
                              >
                          ...
                          const isHighlighted = matchesSearchQuery(
                            player,
                            searchQuery,
                          );
                          ...
                          return (
                            <td
                              ...
                              className={`${styles.playerCell} ${
                                activeTab !== "inactive" && depthIndex === 0
                                  ? styles.starterCell
                                  : ""
                              } ${!isHighlighted ? styles.dimmedCell : ""}`}
                            >
                              <div className={styles.cellContent}>
                                <div className={styles.playerNumName}>
          ...
          return (
            <div key={category} className={styles.mobileCategoryGroup}>
              ...
              {positions.map((position) => {
                ...
                {
                  /* Inactive mobile view */
                }
                if (activeTab === "inactive") {
                  return (
                    <div key={position} className={styles.mobilePosCard}>
                      <div className={styles.mobilePosCardHeader}>
                        {getPositionLabel(position)}
                      </div>

                      <div className={styles.mobilePosCardBody}>
                        {players.map((player) => {
                          const isHighlighted = matchesSearchQuery(
                            player,
                            searchQuery,
                          ...
                          return (
                            ...
                            >
                              <span className={styles.mobileStringPlayer}>
                                <span className={styles.cellNumber}>
                                  #{player.number}
                                </span>{" "}
                                {player.name}
                ...
                {
                  /* Normal depth chart mobile view */
                }
                return (
                  <div key={position} className={styles.mobilePosCard}>
                    <div className={styles.mobilePosCardHeader}>
                    ...
                    <div className={styles.mobilePosCardBody}>
                      {DEPTH_COLUMNS.map((depthIndex) => {
                        const player = players[depthIndex];
                        ...
                        return (
                          <div
                            key={depthIndex}
                            className={`${styles.mobileStringRow} ${
                              depthIndex === 0 ? styles.mobileStarterRow : ""
                            } ${!isHighlighted ? styles.dimmedCell : ""}`}
                          >
                            <span className={styles.mobileStringLabel}>
                              {STRING_LABELS[depthIndex]}
                            </span>
